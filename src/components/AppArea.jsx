...
export default function AppArea() {
  ...
  useEffect(() => { if (session) { loadEntries(); loadPreferences(); checkWhoop(); checkAdmin(); setDailyCalorieGoal(Number.isFinite(Number(session.user.user_metadata?.daily_calorie_goal)) ? Number(session.user.user_metadata.daily_calorie_goal) : null) } }, [session])
