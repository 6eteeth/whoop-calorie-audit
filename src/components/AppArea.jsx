...
export default function AppArea() {
  ...
  const [session, setSession] = useState(null), [loading, setLoading] = useState(true), [entries, setEntries] = useState([]), [tab, setTab] = useState('dashboard'), [editing, setEditing] = useState(emptyEntry()), [message, setMessage] = useState(''), [whoopConnected, setWhoopConnected] = useState(false), [wearableChoice, setWearableChoice] = useState(null), [isAdmin, setIsAdmin] = useState(false), [dailyCalorieGoal, setDailyCalorieGoal] = useState(null)
  ...
  useEffect(() => { if (session) { loadEntries(); loadPreferences(); checkWhoop(); checkAdmin(); loadDailyCalorieGoal() } }, [session])
  async function loadDailyCalorieGoal() { const { data, error } = await supabase.auth.getUser(); if (error) { setDailyCalorieGoal(null); return } const goal = data.user?.user_metadata?.daily_calorie_goal; setDailyCalorieGoal(Number.isFinite(Number(goal)) && Number(goal) > 0 ? Number(goal) : null) }
